...
- **`make configure`** seeds `~/Pictures/ImageGen/.imagegen/settings.json` from 1Password so the app never prompts for a gateway token. This is needed because **WebKit partitions localStorage per executable**: `tauri dev` stores under `~/Library/WebKit/image-gen`, the bundled app under `~/Library/WebKit/com.jkrumm.image-gen`. Settings entered in one are invisible to the other. The file is the store of record; localStorage is a cache hydrated from it on boot.
