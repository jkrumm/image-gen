...
- Brief in → **Plan** (button / Cmd+Enter; never on keystroke). The response renders as:
  ...
  - derived settings pre-filled into the normal controls (size, quality, background, n, moderation) — every one overridable, none hidden; user-pinned values are echoed verbatim, never derived over. Model is normally left `auto` and routed by the gateway (edits and `high`+ finals → `gpt-image-2.5-sunburst`; drafts → `gpt-image-2.5-flare`; `gpt-image-2`/`-1.5`/`-mini` are retired from the generate path, still parsed forever on the read path), and `input_fidelity` is never sent because neither generatable model accepts it — both are locked to high internally;
...
{ "intent": { "detected": "icon", "confidence": 0.93 },
  ...
  "assumptions": ["…"],
  "settings": { "endpoint": "edit", "model": "gpt-image-2.5-sunburst", "size": "1024x1024",
