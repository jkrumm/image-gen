...
14. **~~Uncommitted oxlint-preset fix in basalt-ui.~~ Resolved.** Shipped upstream; this repo now consumes `basalt-ui` from npm at a pinned version, so nothing here depends on an unpushed sibling checkout. The basalt toolchain is scaffolded (`.basalt/manifest.json`, `.claude/rules/basalt-*.md`, `DESIGN.md`, `check-theme` in CI) — refresh it after an upgrade with `bunx basalt-ui sync`.
